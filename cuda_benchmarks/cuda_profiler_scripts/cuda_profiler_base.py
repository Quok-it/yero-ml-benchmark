...
'''
def cuda_profiler_base(queue, stop_event, model, device, inputs, targets, criterion, optimizer,
    ...
    In addition, limiting all NVIDIA GPUs that may have advanced hardware functionality such as tf32/bf16.
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        ...
        with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
            ...
            try:
                ...
                vram_limit_hit = True
                queue.put(f"NO DATA. Model exceeded GPU's VRAM.\n"
                          f"Error: {e}")

            except Exception as e:
                queue.put(f"NO DATA. Model has an issue with the input size or another cause. Check model documentation for input size.\n"
                          f"Error: {e}")
    ...
    if vram_limit_hit != True:
        if edge_model_case != True:
            ...
            profile_output = convert_to_gpu_only_trace(profile_output, benchmark_info, power_draw_list, free_memory_list,
                                                       device_temp_list, skip_count * training_rec)
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        ...
        with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
            ...
            try:
                ...
                queue.put(f"NO DATA. Model has an issue with the input size or another cause. "
                          f"Check model documentation for input size.\n"
                          f"Error: {e}")
                edge_model_case = True
